...
$VMsOff.ForEach({ echo "$_" })
# $VMs.ForEach({ Stop-VM -Name "$_" })
...
foreach( $dest in $DestinationsRemote ) {
    ...
    #Robocopy $DestinationMain $dest /E /B /R:10 /W:30 /LOG+:$Logfile /TEE /NP
}
...
Discord-PostWebhook "$($DiscordTitle)" `
    ":information_source: Backups are complete. The report was sent to `$($EmailTo1)`."
